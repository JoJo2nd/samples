...
RunParams ShaderParams[SOLID_PROGS] = {
    {VS_ASSET_PATH,         FS_ASSET_PATH,             nullptr,                   nullptr,                   nullptr,                 "CPU Z Bin, CPU Tile Light Cull", BGFX_INVALID_HANDLE, BGFX_INVALID_HANDLE, BGFX_INVALID_HANDLE, BGFX_INVALID_HANDLE},
    {VS_ASSET_PATH,         FS_ASSET_PATH, VS_DEBUG_ASSET_PATH, FS_FSDEBUG_WCS_ASSET_PATH,                   nullptr, "CPU Z Bin, CPU Tile Light Cull (Debug Overlay)", BGFX_INVALID_HANDLE, BGFX_INVALID_HANDLE, BGFX_INVALID_HANDLE, BGFX_INVALID_HANDLE},
    {VS_ASSET_PATH,     FS_WCS_ASSET_PATH,             nullptr,                   nullptr, CS_LIGHT_CULL_PATH_NO_OPT,          "CPU Z Bin, GPU Tile Light Cull (Slow)", BGFX_INVALID_HANDLE, BGFX_INVALID_HANDLE, BGFX_INVALID_HANDLE, BGFX_INVALID_HANDLE},
    {VS_ASSET_PATH,     FS_WCS_ASSET_PATH,             nullptr,                   nullptr,        CS_LIGHT_CULL_PATH,                 "CPU Z Bin, GPU Tile Light Cull", BGFX_INVALID_HANDLE, BGFX_INVALID_HANDLE, BGFX_INVALID_HANDLE, BGFX_INVALID_HANDLE},
    {VS_ASSET_PATH,     FS_WCS_ASSET_PATH, VS_DEBUG_ASSET_PATH, FS_FSDEBUG_WCS_ASSET_PATH,        CS_LIGHT_CULL_PATH, "CPU Z Bin, GPU Tile Light Cull (Debug Overlay)", BGFX_INVALID_HANDLE, BGFX_INVALID_HANDLE, BGFX_INVALID_HANDLE, BGFX_INVALID_HANDLE},
    {VS_ASSET_PATH, FS_DEBUG01_ASSET_PATH,             nullptr,                   nullptr,                   nullptr,                              "Z Bin light evals", BGFX_INVALID_HANDLE, BGFX_INVALID_HANDLE, BGFX_INVALID_HANDLE, BGFX_INVALID_HANDLE},
    {VS_ASSET_PATH, FS_DEBUG02_ASSET_PATH,             nullptr,                   nullptr,                   nullptr,                              "Tiled light evals", BGFX_INVALID_HANDLE, BGFX_INVALID_HANDLE, BGFX_INVALID_HANDLE, BGFX_INVALID_HANDLE},
};
...
private:
    ...
    void init(int32_t _argc, const char* const* _argv, uint32_t _width, uint32_t _height) {
        ...
        bgfx::setViewClear(RENDER_PASS_COMPUTE
		...
		for (uint32_t zz = 0, i = 0; zz < INSTANCE_GIRD_SIZE; ++zz)
		{
			for (uint32_t xx = 0; xx < INSTANCE_GIRD_SIZE; ++xx, ++i)
			{
				lights[i].p.x = -15.5f + float(xx)*3.0f;
				lights[i].p.y = 1.5f;
				lights[i].p.z = -15.0f + float(zz)*3.0f;
				lights[i].radius = 2.5f;
		...
		zBinBuffer = bgfx::createDynamicVertexBuffer(Z_BUCKETS*sizeof(uint16_t)*2, zBinVertexDecl, BGFX_BUFFER_COMPUTE_READ | BGFX_BUFFER_COMPUTE_FORMAT_16x2 | BGFX_BUFFER_COMPUTE_TYPE_UINT);
		lightListBuffer = bgfx::createDynamicVertexBuffer(LIGHT_COUNT*LIGHT_COUNT*sizeof(uint16_t), lightIndexVertexDecl, BGFX_BUFFER_COMPUTE_READ | BGFX_BUFFER_COMPUTE_FORMAT_16x1 | BGFX_BUFFER_COMPUTE_TYPE_UINT);

        // Each grid cell can have upto MAX_LIGHT_GRID_COUNT lights
        lightGridFatBuffer	= bgfx::createDynamicVertexBuffer(LIGHT_GRID_SPACE*MAX_LIGHT_GRID_COUNT, lightIndexVertexDecl, BGFX_BUFFER_COMPUTE_READ_WRITE | BGFX_BUFFER_COMPUTE_FORMAT_16x1 | BGFX_BUFFER_COMPUTE_TYPE_UINT);
		lightGridBuffer		= bgfx::createDynamicVertexBuffer(LIGHT_GRID_SPACE*MAX_LIGHT_GRID_COUNT, lightIndexVertexDecl, BGFX_BUFFER_COMPUTE_READ | BGFX_BUFFER_COMPUTE_FORMAT_16x1 | BGFX_BUFFER_COMPUTE_TYPE_UINT);
	...
	void zbinLights(LightInfo* viewSpaceLights, CameraParams const* cam, LightInfo* lights, uint32_t light_count, uint32_t* zbin, float* zstep) {
		...
		*zstep = z_step;
		...
		for (; cur_l < end_l; ++cur_l) {
			uint16_t min_z = (uint16_t)MIN((viewSpaceLights[cur_l].p.z - viewSpaceLights[cur_l].radius) / z_step, Z_BUCKETS-1);
			uint16_t max_z = (uint16_t)MIN((viewSpaceLights[cur_l].p.z + viewSpaceLights[cur_l].radius) / z_step, Z_BUCKETS-1);
	...
	void extractFrustumPlanes(CameraParams const* cam, plane_t frustum[6]) {
		...
		vec3_add(&ntl, &vnear, &vnup); vec3_sub(&ntl, &vnright);
		vec3_add(&ntr, &vnear, &vnup); vec3_add(&ntr, &vnright);
		vec3_sub(&nbl, &vnear, &vnup); vec3_sub(&nbl, &vnright);
		vec3_sub(&nbr, &vnear, &vnup); vec3_add(&nbr, &vnright);

		vec3_add(&ftl, &vfar, &vfup); vec3_sub(&ftl, &vfright);
		vec3_add(&ftr, &vfar, &vfup); vec3_add(&ftr, &vfright);
		vec3_sub(&fbl, &vfar, &vfup); vec3_sub(&fbl, &vfright);
		vec3_sub(&fbr, &vfar, &vfup); vec3_add(&fbr, &vfright);
	...
	void updateLightingGrid(LightInfo* lights, uint32_t light_count, CameraParams const* cam, uint16_t* grid, uint16_t* debugLightList) {
		...
		for (uint32_t y = 0; y < m_height; y += GRID_SIZE) {
			...
			for (uint32_t x = 0; x < m_width; x += GRID_SIZE, ++grid_idx) {
				...
				vec3_add(&ntl, &v_near_c, &vnup); vec3_sub(&ntl, &vnright);
				vec3_add(&ntr, &v_near_c, &vnup); vec3_add(&ntr, &vnright);
				vec3_sub(&nbl, &v_near_c, &vnup); vec3_sub(&nbl, &vnright);
				vec3_sub(&nbr, &v_near_c, &vnup); vec3_add(&nbr, &vnright);

				vec3_add(&ftl, &v_far_c, &vfup); vec3_sub(&ftl, &vfright);
				vec3_add(&ftr, &v_far_c, &vfup); vec3_add(&ftr, &vfright);
				vec3_sub(&fbl, &v_far_c, &vfup); vec3_sub(&fbl, &vfright);
				vec3_sub(&fbr, &v_far_c, &vfup); vec3_add(&fbr, &vfright);
				...
				if (grid_idx == dbgCell) {
					...
					//	bgfx::dbgTextPrintf(0, 30 + pp, 0x6f, "%f, %f, %f, %f", frustum[pp].n.x, frustum[pp].n.y, frustum[pp].n.z, frustum[pp].d);
					//}
				...
				if (dbgLightGridFrustum && grid_idx == dbgCell) {
					...
					ddMoveTo(wtl.x, wtl.y, wtl.z); ddLineTo(wtl2.x, wtl2.y, wtl2.z);
					ddMoveTo(wtr.x, wtr.y, wtr.z); ddLineTo(wtr2.x, wtr2.y, wtr2.z);
					ddMoveTo(wbl.x, wbl.y, wbl.z); ddLineTo(wbl2.x, wbl2.y, wbl2.z);
					ddMoveTo(wbr.x, wbr.y, wbr.z); ddLineTo(wbr2.x, wbr2.y, wbr2.z);
				}
    ...
    bool update() {
        ...
        if (!entry::processEvents(m_width, m_height, m_debug, m_reset, &mouseState)) {
			for (auto& i : inputs) {
				if (i.state && !i.down) { i.rising = true; }
			...
			if (inputs[IA_Shader].rising) { dbgShader = (dbgShader + 1) % SOLID_PROGS;  }
            if (inputs[IA_ShaderPrev].rising) { dbgShader = !dbgShader ? SOLID_PROGS-1 : --dbgShader;  }
			...
			float lightingParams[4] = { zstep, (float)GRID_SIZE, (float)((m_width + (GRID_SIZE_M_ONE)) / GRID_SIZE), (float)(m_height) };
            ...
            if (enableCPUUpdateLightGrid) {
                bgfx::updateDynamicVertexBuffer(lightGridBuffer, 0, bgfx::copy(grid, LIGHT_GRID_SPACE*sizeof(uint16_t)*MAX_LIGHT_GRID_COUNT));
            }
            ...
            if (enableComputeLightCull) {
    			float viewParams[8] = { 
                    saved_cam_params.nearPlane, saved_cam_params.farPlane, saved_cam_params.fovy, saved_cam_params.aspect,
                    (float)LIGHT_COUNT, (float)m_width, (float)m_height, 0
                };
    			...
    			bgfx::dispatch(RENDER_PASS_COMPUTE, ShaderParams[dbgShader].csLightCull, ((m_width + (GRID_SIZE_M_ONE)) / GRID_SIZE), ((m_height + (GRID_SIZE_M_ONE)) / GRID_SIZE), 1);
            }
